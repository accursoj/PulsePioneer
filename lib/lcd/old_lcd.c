...
static void new_lvgl_flush_cb(lv_display_t *disp, const lv_area_t *area, uint8_t *color_map) {
...
static void new_init_lvgl(void) {
    if (_TESTING) ESP_LOGI(TAG, "In new_init_lvgl()");
    lv_init();
    ...
    lv_display_set_flush_cb(disp,new_lvgl_flush_cb);
}

static void new_init_st7796(void) {
    if (_TESTING) ESP_LOGI(TAG, "In new_init_st7796()");
...
void new_init_lcd() {
    if (_TESTING) ESP_LOGI(TAG, "In new_init_lcd()");
